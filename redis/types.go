...
func (rds *RedisDataStructure) Set(key []byte, ttl time.Duration, value []byte) error {
	...
	// binary.MaxVarintLen64 的值是 10，是因为它表示的是 Varint 编码的最大长度，而不是 int64 的字节长度。
	//在 Varint 编码中，为了表示一个 64 位整数，可能需要使用不同长度的字节，具体取决于整数的大小。
	//通常情况下，较小的整数使用较少的字节，但在最坏情况下，为了编码一个 64 位整数，可能需要 10 个字节。
	...
	//这行代码的目的是将过期时间 expire 编码为可变长度整数，
	//并将编码后的数据写入到 buf 的指定位置，然后更新 index 以指示下一个写入操作的位置。
